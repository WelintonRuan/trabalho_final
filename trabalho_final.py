...
def verificar_nome_existente(nome):
    ...
    try:
        cursor = conexao_global.cursor()
        sql = "SELECT id FROM alunos WHERE nome = %s"
...
def cadastrar_professor():  
    ...
    while True:
        try:
            ...
            elif materia not in materias_validas:
                print(f" Matéria inválida. Matérias permitidas: {', '.join(materias_validas)}")
                continue
           
            elif not nome.replace(" ", "").isalpha():
                print(" Nome pode ter apenas letras e espaços.")
                continue
...
def cadastrar_aluno():
    while True:
        try:
            ...
            nome1 = input("Nome do aluno: ").strip()
        
            if not nome1:
                print("Nome não pode ser vazio.")
                continue

            elif not nome1.replace(" ","").isalpha():
                print("Utilize apenas letras ou espaços.")
                continue

            if verificar_nome_existente(nome1):
                print("Já existe um aluno com esse nome, Adicione sobrenome ao aluno.")
                continue
